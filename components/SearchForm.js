...
const SearchForm = (props) => {
	...
	const getPokemonApi = async () => {
		const pokemon = await apiCall(['pokedex/1'])
			.then(response => {
				return response.length && setPokemonNames(response[0]);
			})
	...
	const predictiveSearch = (value) => {
		...
		if (queryItem.length > 2) setFilterList(pokemon.filter(el => el.includes(queryItem)));
		else setFilterList([]);
	}
	...
	return (
		<form className={styles.search__form} action="" onSubmit={(e) => searchEvent(e)}>
			...
			{filterList.length && <DropdownList items={filterList} />}
		</form>
    );
};
